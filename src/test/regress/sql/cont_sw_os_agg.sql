CREATE STREAM test_sw_os_stream ();
...
CREATE CONTINUOUS VIEW test_sw_pc2 AS SELECT percentile_cont(0.67) WITHIN GROUP (ORDER BY x::integer) + percentile_cont(0.88) WITHIN GROUP (ORDER BY z::integer) FROM test_sw_os_stream WHERE (arrival_timestamp > clock_timestamp() - interval '60 second');
