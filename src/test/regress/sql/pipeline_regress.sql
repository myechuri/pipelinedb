CREATE STREAM test_regress_stream (
...
CREATE CONTINUOUS VIEW test_view AS
  SELECT
    ...
    foobar,
    COUNT(*) AS count
  FROM test_regress_stream
  WHERE (SECOND(arrival_timestamp) > clock_timestamp() - interval '7 day')
  GROUP BY
    secondstamp,
    foobar;

INSERT INTO test_regress_stream (foobar, ts) VALUES ('foo', clock_timestamp()), ('foo', clock_timestamp());
SELECT pg_sleep(1);
INSERT INTO test_regress_stream (foobar, ts) VALUES ('foo', clock_timestamp()), ('bar', clock_timestamp());

SELECT foobar, count FROM test_view ORDER BY secondstamp, foobar;

DROP CONTINUOUS VIEW test_view;
DROP STREAM test_regress_stream;
...
INSERT INTO test_regress_stream (user_id, page_id) VALUES (1, 1);
SELECT pg_sleep(1);
INSERT INTO test_regress_stream (user_id, page_id) VALUES (2, 2);
...
CREATE CONTINUOUS VIEW test_view AS
  SELECT arrival_timestamp FROM test_regress_stream
  WHERE arrival_timestamp > clock_timestamp() - interval '1 hour';

INSERT INTO test_regress_stream (x) VALUES (NULL);
INSERT INTO test_regress_stream (x) VALUES (NULL);
...
INSERT INTO test_regress_stream (id) VALUES (1), (2), (3);
...
INSERT INTO test_regress_stream (id, x) VALUES (1, 1.5), (2, 3.0), (3, 4.5);
