...
FROM stream_cqminmax GROUP BY key;

INSERT INTO stream_cqminmax
...
SELECT key, i8min, i8max, i4min, i4max, i2min, i2max FROM test_min_max ORDER BY key;
SELECT key, omin, omax, f8min, f8max, f4min, f4max FROM test_min_max ORDER BY key;
SELECT key, dmin, dmax, tmin, tmax, tzmin, tzmax, mmin FROM test_min_max ORDER BY key;
...
DROP STREAM stream_cqminmax CASCADE;
